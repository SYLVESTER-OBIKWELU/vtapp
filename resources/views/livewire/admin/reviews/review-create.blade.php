<div class="flex h-full w-full flex-1 flex-col gap-6 p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Add Review</h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1">Create a new testimonial</p>
        </div>
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <div class="glass rounded-2xl p-6">
            ...
            <div class="bg-slate-800 rounded-xl p-8 text-center max-w-md mx-auto">
                <!-- Quote Icon -->
                <div
                    class="w-16 h-16 mx-auto mb-6 rounded-full bg-gradient-to-r {{ $gradient_color }} flex items-center justify-center">
                ...
                <div class="relative w-24 h-24 mx-auto mb-6">
                    <div class="absolute inset-0 bg-gradient-to-r {{ $gradient_color }} rounded-full animate-pulse">
                    </div>
                    @if ($reviewer_image)
                    <img src="{{ $reviewer_image->temporaryUrl() }}"
                        class="relative w-full h-full object-cover rounded-full border-4 border-slate-700">
                    @else
                    <div
                        class="relative w-full h-full rounded-full bg-gradient-to-r {{ $gradient_color }} flex items-center justify-center text-white text-2xl font-bold border-4 border-slate-700">
                        {{ strtoupper(substr($reviewer_name ?: 'R', 0, 1)) }}
                    </div>
                    @endif
                </div>

                <!-- Name & Company -->
                <h3 class="text-xl font-bold text-white mb-1">{{ $reviewer_name ?: 'Reviewer Name' }}</h3>
                <p class="text-cyan-400 text-sm mb-4">{{ $company_name ?: 'Company Name' }}</p>

                <!-- Stars -->
                <div class="flex justify-center gap-1 mb-4">
                    @for ($i = 1; $i <= 5; $i++) <svg
                        class="w-5 h-5 {{ $i <= $rating ? 'text-yellow-400' : 'text-gray-600' }}" fill="currentColor"
                        viewBox="0 0 20 20">
                        <path
                            d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z">
                        </path>
                ...
                <p class="text-gray-300 leading-relaxed text-sm">
                    "{{ $review_text ?: 'Your testimonial text will appear here...' }}"
                </p>
                @if ($company_tagline)
                <p class="text-gray-500 text-xs mt-4 italic">— {{ $company_tagline }}</p>
                @endif
            </div>
        </div>
        ...
        <div class="flex justify-end gap-4">
            ...
            <button type="submit"
                ...
                <span wire:loading.remove wire:target="save">Create Review</span>
